...
public class Greedy implements SearchingAlgorithm {
    ...
    public Greedy(Character subject, int[] start, int[] end) {
    ...
    private void constructPath(PathFinder.Node end) {
        while (end.prev != null) {
            path.add(0, end);
            end = end.prev;
        }
    ...
    @Override
    public List<PathFinder.Node> run() {
        setStartPoint(subject.getPosition());
        path.clear();

        Set<String> visited = new HashSet<>();
        PriorityQueue<PathFinder.Node> toExplore = new PriorityQueue<>((a, b) -> (int) (a.dist - b.dist));
        if (negation)
            toExplore = new PriorityQueue<>((a, b) -> (int) (b.dist - a.dist));
        toExplore.add(new PathFinder.Node(start[0], start[1], getHeuristic(start[0], start[1])));

        while (!toExplore.isEmpty()) {
            PathFinder.Node curr = toExplore.poll();
